...
<?php
    ...
    $queryActive = "UPDATE banners SET active = ? WHERE id = ? ";
    $stmt = mysqli_prepare($conn, $queryActive);
    mysqli_stmt_bind_param($stmt, "si", $activeChange, $id);
    $result = mysqli_stmt_execute($stmt);

    // Query all banners show data in the table
    $query = queryAll('banners', '(active =' . IN_ACTIVE . ' OR active = ' . ACTIVE . ')', 'id', 'DESC');
                                ...
                                <table class="table table-striped table-bordered">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="td-actions table-text" style="width:15%">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                        ...
                                        ?>
                                                <tr>
                                                    <td><?php echo '<img src="../theme/upload/banner/' . $row['image_url'] . ' " width="100px">' ?></td>
                                                    ...
                                                    <td class="td-actions">
                                                        <a href="edit_banner.php?id=<?php echo $row['id']; ?>" class="btn btn-table-edit btn-small btn-warning" style="margin-left: 35px;"><i class="icon-edit"> </i></a>
                                                        <a onclick="return confirm('Do you want to delete ?')" href="delete_soft_banner.php?id=<?php echo $row['id']; ?>" class="btn  btn-table-delete btn-danger btn-small"><i class="btn-icon-only icon-remove"> </i></a>
                                                    </td>
                                                </tr>
                                            <?php }
                                        } else {
