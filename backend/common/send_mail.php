...
function resetPassword($getUserName, $getEmail, $token)
{
    $fullUrl = (isset($_SERVER['HTTP_REFERER'])) ? $_SERVER['HTTP_REFERER'] : '';
    $arrUrl = explode('backend', $fullUrl);
    $lastUrl = $arrUrl[0] . 'backend/auth/reset_password.php?token=' . $token . '&email=' . $getEmail;;
    ...
    try {
        ...
        $mail->setFrom($toEmail, $getUserName);
