...
<body>
    ...
    $id = $_GET['id'];
    getCheckIdUrl("list_customer.php", $id);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($errors)) {
            ...
            $query = "SELECT * FROM customers WHERE (`name` = ? OR `email` = ?) AND id != ?";
            ...
            if ($numRows > 0) {
                ...
                // Customer no change data, show message no data changed phone
                if ($name == $record['name']  && $email == $record['email'] && $phone == $record['phone'] && $passwordMd5 == $record['password'] && $active == $record['active']) {
                    ...
                    // Customer no change new password then update old password
                    if ($passwordMd5 == $record['password']) {
                        $stmtUpdate = mysqli_prepare($conn, "UPDATE `customers` SET `name` = ?, `email` = ?, `phone` = ?, `password` = ?, `active` = ? WHERE id = ?");
                        mysqli_stmt_bind_param($stmtUpdate, "sssssi", $name, $email, $phone, $passwordMd5, $active, $id);
                    } else {

                        // Customer change data and change new password , then update new encrypt password in the table customer
                        $newPasswordMd5 = md5($password);
                        $stmtUpdate = mysqli_prepare($conn, "UPDATE `customers` SET `name` = ?, `email` = ?, `phone` = ?, `password` = ?, `active` = ? WHERE id = ?");
                        mysqli_stmt_bind_param($stmtUpdate, "sssssi", $name, $email, $phone, $newPasswordMd5, $active, $id);
                    }
                    ...
                    if ($resultUpdate) {
                        ...
                        $_SESSION['flash_message'] = [
                            'type' => 'error',
                            'message' => 'Error: Cannot update customer'
                        ];
                    }
                }
            }
                                ...
                                <div class="tab-pane" id="formcontrols">
                                    <form id="" class="form-horizontal" method="POST">
                                        ...
                                        <div class="control-group">
                                            <label class="control-label" for="user_name">Name <span class="color-red">*</span> </label>
                                            <div class="controls">
                                        ...
                                        <div class="control-group">
                                            ...
                                            <div class="controls">
                                                <input type="text" class="span6" id="phone" name="phone" value="<?php echo (isset($record['phone'])) ? $record['phone'] : ''; ?>">
